...
var _apply_auths_serially = function (self, db, auths, callback) {
    ...
    if (auth.authMechanism == 'GSSAPI') {
        ...
        if (process.platform == 'win32') {
            mongodb_sspi_authenticate(db, auth.email, auth.password, auth.authdb, options, callback);
        } else {
            mongodb_gssapi_authenticate(db, auth.email, auth.password, auth.authdb, options, callback);
        }
    } else if (auth.authMechanism == 'MONGODB-CR') {
        mongodb_cr_authenticate(db, auth.email, auth.password, auth.authdb, options, callback);
    } else if (auth.authMechanism == 'PLAIN') {
        mongodb_plain_authenticate(db, auth.email, auth.password, auth.authdb, options, callback);
    } else if (auth.authMechanism == 'MONGODB-X509') {
        mongodb_x509_authenticate(db, auth.email, auth.password, auth.authdb, options, callback);
    }
}
